...
function documentation.
4) compute_beta: computes the false negative rate: the probability to miss an underlying difference of size epsilon,
...
def welch_test(data1, data2, alpha=0.05, alternative='two-sided'):
    """
    ...
    Alternative hypothesis:
        'greater': mean(data1) > mean(data2)
        'less': mean(data1) < mean(data2)
        'two-sided': mean(data1) != mean(data2)
    """
    ...
    t, p = stats.ttest_ind(data1, data2, equal_var=False, alternative=alternative)

    if p <= alpha:
        if alternative == 'greater':
            print("\n\nResult of the Welch's t-test at level %02g: μ1>μ2, the test passed with p-value = %02g." % (alpha, p))
        ...
        if alternative == 'greater':
            print("\n\nResults of the Welch's t-test at level %02g: there is not enough evidence to prove μ1>μ2." % alpha)
        elif alternative == 'less':
            print("\n\nResults of the Welch's t-test at level %02g: there is not enough evidence to prove μ2>μ1." % alpha)
        else:
            print("\n\nResults of the Welch's t-test at level %02g: there is not enough evidence to prove any difference between μ1 and μ2." % alpha)
...
def bootstrap_test(data1, data2, alpha=0.05):
    ...
    The performance measures of Algo2.
    - alpha (float in ]0,1[)
    The significance level used by the bootstrap test.
    """
    ...
    if decision:
        if np.sign(res.upper_bound) < 0:
            ...
            print("\n\nResult of the bootstrap test at level %02g: μ1>μ2, the test passed with a confidence interval μ1-μ2 in %02g, %02g."
                  % (alpha, res.lower_bound, res.upper_bound))
    else:
        print("\n\nResults of the bootstrap test at level %02g: there is not enough evidence to prove any order relation between μ1 and μ2." % alpha)

    print("Bootstrap test done.")


def empirical_false_pos_rate(data, alpha=0.05, alternative='two-sided'):
    """
    Compute and plot empirical estimates of the probability of type-I error given a list of performance measures.
    If this list is of size N_data, this is done for N=2:floor(N_data/2).
    Two different tests are used: the bootstrap confidence interval test and the Welch's t-test.
    ...
    - alternative ('greater', 'less' or 'two-sided')
    Alternative hypothesis used by the Welch's t-test.
    """
    ...
    for i_n, n in enumerate(sizes):
        ...
        for rep in range(nb_reps):
            ...
            results[rep, i_n, 0] = stats.ttest_ind(sample_1, sample_2, equal_var=False, alternative=alternative)[1] < alpha
    ...
    plt.legend([u'α=%02g' % alpha] + ["Welch's $t$-test", 'bootstrap test'])
    plt.xlabel('sample size (N)')
    plt.ylabel('P(false positive)')
    plt.title(u'Estimation of type-I error rate as a function of $N$ when $α=%02g$' % alpha)
...
def compute_beta(epsilon, sample_size, alpha=0.05, data1=None, data2=None, s1=None, s2=None,
                 beta_requirement=0.2, alternative='two-sided'):
    """
    Computes the probability of type-II error beta to detect an effect size epsilon
    when testing for a difference between performances of Algo1 versus Algo2,
    using a Welch's t-test with significance alpha and sample size N.

    Params
    ------
    - epsilon (int, float or list of int or float)
    The magnitude of the effect size one wants to be able to detect.
    ...
    Requirement on the value of beta.
    - alternative ('greater', 'less' or 'two-sided')
    Alternative hypothesis used by the final Welch's t-test.

    Notes
    -----
    epsilon is always interpreted as a positive magnitude.

    For alternative='greater', the underlying effect is μ1-μ2 = +epsilon.
    For alternative='less', the underlying effect is μ1-μ2 = -epsilon.
    For alternative='two-sided', only the magnitude matters.
    """
    ...
    assert data2 is not None or s2 is not None, "data2 or s2 should be provided"
    ...
    for i_diff, eps in enumerate(epsilon):
        assert eps >= 0, "epsilon should represent a positive effect magnitude."
        ...
        for i_n, n in enumerate(sample_size):
            ...
            if alternative in ['greater', 'less']:
                # For a one-sided test, beta is identical for 'greater' and 'less'
                # when epsilon represents the magnitude of the effect in the hypothesized direction.
                t_crit = t_dist.ppf(1 - alpha, nu)
                beta = t_dist.cdf(t_crit - t_eps, nu)

            else:
                # For a two-sided test, failure to reject occurs when the shifted
                # test statistic falls between the two critical values.
                t_crit = t_dist.ppf(1 - alpha / 2., nu)
                beta = t_dist.cdf(t_crit - t_eps, nu) - t_dist.cdf(-t_crit - t_eps, nu)

            results[i_n, i_diff] = beta
        ...
        if not sample_size_found:
            selected_sample_size.append('>' + str(sample_size[-1]))
...
def plot_beta(beta, epsilon, sample_size, beta_requirement=0.2):
    ...
    if n_eps > 1:
        legend += [u'ε = %0g' % epsilon[i] for i in range(n_eps)]
    else:
        legend += [u'ε = %0g' % epsilon[0]]
...
def main():
    ...
    # Alternative hypothesis for Welch's test and the associated power calculation.
    # 'greater': mean(data1) > mean(data2)
    # 'less': mean(data1) < mean(data2)
    # 'two-sided': mean(data1) != mean(data2)
    ...
    welch_test(data1, data2, alpha, alternative=alternative)
    bootstrap_test(data1, data2, alpha)
    empirical_false_pos_rate(data1, alpha, alternative=alternative)

    beta = compute_beta(epsilon, sample_size, alpha, data1, data2,
                        beta_requirement=beta_requirement, alternative=alternative)
